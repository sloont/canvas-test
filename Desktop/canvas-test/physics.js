...
const checkMouseClick = (ballA, mouse) => {
...
const applyForceWithClick = (ballA, mouse) => {
    let normal = [(mouse.x - ballA.xpos), (mouse.y - ballA.ypos)];
    const magnitude = Math.sqrt((normal[0] * normal[0]) + (normal[1] * normal[1]));
    normal = [(normal[0] / magnitude) , (normal[1] / magnitude)];

    relativeVelocity = [ballA.dx, ballA.dy];
    velocityOnNormal = (relativeVelocity[0] * normal[0]) + (relativeVelocity[1] * normal[1]);
